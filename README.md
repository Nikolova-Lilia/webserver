...
#include <cstring>    // for std::memset and strcmp-like functions
#include <sys/socket.h> // socket(), bind(), listen(), accept()
#include <netinet/in.h> // sockaddr_in
#include <unistd.h>     // close(), read(), write()
#include <csignal>      // signal()
📌 signal(SIGINT, sigint_handler);
When you press Ctrl+C, this triggers sigint_handler() to cleanly shut down your server.

🛠 get_mime_type(const std::string &filename)
Simple helper that returns the MIME type based on the file extension (.html, .css, etc.).
Used in buildResponse().

✅ Main server code (will map this to your structure):
cpp
Copy
Edit
int g_server_fd = -1;
Global socket fd so we can close it later.
...
Allow reusing the port if the server restarts quickly (not strictly required, but good practice).
...
This defines the server address and port:
...
bind()
Tells the OS: “I want this socket to listen on port 8080.”
...
listen()
Tells the socket to listen for incoming connections (10 clients max in the queue).
...
accept()
Waits for a client to connect. Returns a new socket client_fd used to talk to that client.
...
buffer[bytes_read] = '\0';
recv()
Reads raw HTTP request data into buffer.
...
handleRequest()
If the client asks for /, load index.html.
Otherwise, strip the leading / and try to open the requested file.
...
Check if the file exists. If not → send 404 response.
...
Can be optimized, but fine for small files.
...
resp << content;
buildResponse()
Formats the full HTTP response with headers and body.
...
Your Structure	What It Does	In Code
getConfig()	Load config (port, root)	🔸 You can add config reading from a file or cmd args
setupSocket()	socket() + setsockopt()	socket(...)
bind()	Binds socket to address/port	bind(...)
listen()	Marks socket as ready to accept	listen(...)
accept()	Accepts incoming client	accept(...)
recv()	Reads HTTP request from client	read(...)
parseRequest()	Parses request line	std::getline(...) and std::istringstream
handleRequest()	Validates method, finds file, etc.	if (!file.good()) {...}
buildResponse()	Constructs HTTP response with headers	std::ostringstream with status, content-type, length
send()	Sends response to client	send(...)

🧱 What's Next?
You can start breaking the current code into separate functions:
...
This matches your architecture and makes the project much easier to scale, debug, and test.
